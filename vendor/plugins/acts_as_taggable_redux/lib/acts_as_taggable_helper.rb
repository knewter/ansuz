module ActsAsTaggableHelper
  ...
  def link_to_tag(tag, options={})
    options[:url] ||= :tag_url
    link_to(tag.name, self.send(options[:url], tag), :rel => 'tag')
  end
  ...
  def tag_cloud(options = {})
    options.assert_valid_keys(:limit, :conditions, :sort, :url)
    options[:url] ||= :tag_url
    ...
    tags.each do |tag|
      html << %(    <li>)
      html << link_to(tag.name, self.send(url, tag), :class => classes[(tag.taggings_count - min) / divisor]) 
      html << %(</li> \n)
    end
    html <<   %(  </ul>\n)
    html <<   %(</div>\n)
