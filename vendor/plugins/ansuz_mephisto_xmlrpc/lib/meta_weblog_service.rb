class MetaWeblogService < XmlRpcService
  ...
  def getPost(postid, username, password)
    article = Ansuz::JAdams::BlogPost.for(@user).find(postid)
    article_dto_from(article)
  end

  def getRecentPosts(blogid, username, password, numberOfPosts)
    Ansuz::JAdams::BlogPost.for(@user).find(:all, :order => "created_at DESC", :limit => numberOfPosts).collect{ |c| article_dto_from(c) }
  end

  def newPost(blogid, username, password, struct, publish)
    article = Ansuz::JAdams::BlogPost.new
  ...
  def article_dto_from(article)
    MetaWeblogStructs::Article.new(
      ...
      :categories        => [],
      :mt_text_more      => article.contents.to_s,
  ...
  protected
    ...
    def post_it(article, user, password, struct, publish)
      ...
      article.attributes = {:updater => @user, :contents => struct['description'].to_s, :title => struct['title'].to_s}
      ...
      utc_date = Time.utc(struct['dateCreated'].year, struct['dateCreated'].month, struct['dateCreated'].day, struct['dateCreated'].hour, struct['dateCreated'].sec, struct['dateCreated'].min) rescue Time.now.utc
